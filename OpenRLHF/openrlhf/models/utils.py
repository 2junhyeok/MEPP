...
def normalize_score(
    ...
    if score_mode != "mean":
        raise ValueError(f"Unknown score_mode={score_mode!r}; expected 'sum' or 'mean'.")
    if chosen_lengths is None or rejected_lengths is None:
        raise ValueError("chosen_lengths and rejected_lengths are required when score_mode='mean'.")
    
    chosen_lengths_calc = chosen_lengths.to(chosen_scores.device).clamp(min=1).to(chosen_scores.dtype)
