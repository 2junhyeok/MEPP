...
from openrlhf.trainer.mepp_trainer import MEPPTrainer
...
if __name__ == "__main__":
    ...
    #* MEPP (Maximum Entropy Preference Projection)
    parser.add_argument(
        "--model.rho",
        type=float,
        default=0.8,
        help="Target confidence level in (0.5, 1) that MEPP pushes pairs up to; pairs already at "
        "least this confident get ~zero gradient.",
    )
    parser.add_argument(
        "--model.score_mode",
        type=str,
        default="mean",
        choices=["sum", "mean"],
        help="'sum' scores a response by its summed log-prob; 'mean' length-normalizes first.",
    )
    parser.add_argument(
        "--model.reduction",
        type=str,
        ...
        help="Batch reduction applied to the per-pair MEPP loss.",
    )
